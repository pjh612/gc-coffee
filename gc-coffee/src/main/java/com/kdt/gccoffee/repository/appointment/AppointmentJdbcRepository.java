...
public class AppointmentJdbcRepository implements AppointmentRepository {
    ...
    public void deleteByAppointmentId(UUID appointmentId) {
        int update = jdbcTemplate.update("DELETE FROM appointments WHERE appointment_id = UNHEX(REPLACE(:appointmentId, '-', ''))"
                , Collections.singletonMap("appointmentId", toUUID(appointmentId.toString().getBytes())));
    ...
    public Optional<Appointment> findByAppointmentId(UUID appointmentId) {
        return Optional.ofNullable(
                jdbcTemplate.queryForObject("SELECT * FROM appointments WHERE appointment_id = UNHEX(REPLACE(:appointmentId, '-', ''))",
                        Collections.singletonMap("appointmentId", toUUID(appointmentId.toString().getBytes())), appointmentRowMapper
                )
        );
    }
    ...
    public Optional<Appointment> findByCustomerId(UUID customerId) {
        return Optional.ofNullable(
                jdbcTemplate.queryForObject("SELECT * FROM appointments WHERE customer_id = UNHEX(REPLACE(:customerId, '-', ''))",
                        Collections.singletonMap("appointmentId", toUUID(customerId.toString().getBytes())), appointmentRowMapper
                )
        );
    }
    ...
    public Optional<Appointment> findByDesignerId(UUID designerId) {
        return Optional.ofNullable(
                jdbcTemplate.queryForObject("SELECT * FROM appointments WHERE designer_id = UNHEX(REPLACE(:designerId, '-', ''))",
                        Collections.singletonMap("appointmentId", toUUID(designerId.toString().getBytes())), appointmentRowMapper
                )
        );
    }
    ...
    public Appointment updateByAppointmentId(Appointment appointment) {
        int update = jdbcTemplate.update("UPDATE appointments SET menu_id = UNHEX(REPLACE(:id, '-', '')), appointed_at = :appintedAt WHERE appointment_id = :appointmentId",
                toParamMap(appointment));
    ...
    private Map<String, Object> toParamMap(Appointment appointment) {
        ...
        paramMap.put("appointmentId", appointment.getAppointmentId());
        paramMap.put("customerId", appointment.getCustomerId());
        paramMap.put("menuId", appointment.getMenuId());
        paramMap.put("designerId", appointment.getDesignerId());
